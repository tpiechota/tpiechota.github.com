$(document).ready(function() {
	...
	function getPortfolioHeight() {
		var folioHeight = $("#portfolio").height();
		return folioHeight;
	}
	...
	function getSize() {		
		...
		var headerHeight = $('div#contact .header').height();		
		
		// Change the divs to that width and height			
		if(getAboutHeight() < winHeight){
			$('#about').css({'min-height' : divHeight+'px'});
		}
		...
		$('#contact[data-full="true"]').css({'width' : (winWidth-containerPadding)+'px', 'height' : winHeight-200+'px'});
		//$('section.parallax-2').css({'height' : divHeight-headerHeight-getNavHeight()+'px'});
		...
		var homesocial = (divHeight*2) - diffr;
		...
		if(currentPage < winHeight) {
			// Add correct classes, repeat for different sizes
			$('ul.nav li > a[href="#home"]').parent().addClass('active');
		} else if(currentPage >= divHeight && currentPage < homesocial) {
			$('ul.nav li > a[href="#social"]').parent().addClass('active');
		} else if(currentPage >= homesocial && currentPage < homesocial + getAboutHeight()) {
			$('ul.nav li > a[href="#about"]').parent().addClass('active');
		} else if(currentPage >= homesocial + getAboutHeight() && currentPage < homesocial + getAboutHeight() + getPortfolioHeight()) {
			$('ul.nav li > a[href="#portfolio"]').parent().addClass('active');
		} else if(currentPage >= homesocial + getAboutHeight() + getPortfolioHeight() && currentPage < homesocial + getAboutHeight() + getPortfolioHeight() + divHeight) {
			$('ul.nav li > a[href="#contact"]').parent().addClass('active');
		}
	}	
	
	// When the user clicks a #header anchor
	$('ul.nav a, .navbar-brand, #footer .arrow-up a').click(function() {
